...
readRuleTreeFromExcel swTrace excelFilePath = do
    ...
    case excelResult of
        ...
        Right rows -> do
            let
                rulesRows = parseRowsToRulesRows swTrace rows
...
readExcelRows swTrace filePath = do
    let
        ...
        extractRowsFromSheet sheetData =
            let
                cells = DataMap.toList (sheetData Lens.^. XLSX.wsCells)
        ...
        groupRowsByRowNumber cells =
            let
                grouped = DataMap.fromListWith DataMap.union [(row, DataMap.singleton col cell) | ((row, col), cell) <- cells]
                colIndices = [col | (_, col) <- map fst cells]
                minCol = minimum colIndices
                maxCol = maximum colIndices
                fillRow rowMap =
                    [ DataMap.findWithDefault
                        XLSX.Cell
            ...
            case result of
                ...
                Right bs -> do
                    let
                        xlsx = XLSX.toXlsx bs
                    let
                        sheets = xlsx Lens.^. XLSX.xlSheets
                    if null sheets
                        then return $ Left "No sheets found in Excel file."
                        else do
                            -- Combine data from all sheets
                            let
                                allRows = concatMap (extractRowsFromSheet . snd) sheets
...
parseRowsToRulesRows swTrace rows =
    let
        ...
        createRulePath row =
            let
                components = [csvTx row, csvScript row, csvRedeemer row, csvGroup row, csvPath row]
            ...
            in
                DataList.intercalate "." nonEmptyComponents
        -----------------
        ...
        isValidCSVRow row =
            ...
            in
                swUse
                    && not (tx == "TX NAME" || script == "SCRIPT")
                    && not (null tx) -- Filter out header rows
                    && (not (null script) || (null script && null redeemer && null group && null path)) -- tx must always be set
                    && (not (null redeemer) || (null redeemer && null group && null path)) -- Script is optional if all subsequent fields are empty
                    && (not (null group) || (null group && null path)) -- Redeemer is optional if subsequent fields are empty
                    && (not (null path) || null path) -- Group is optional if subsequent field (path) is empty
                    -- Path can be empty without conditions
                    -----------------
        ...
        readExpectedOutcome "TestFailure" errMsg = TestFailure (DataList.splitOn ":|:" errMsg)
        ...
        buildRuleTree rules' =
            let
                ...
                insertRule (path, rule) tree =
                    let
                        pathParts = DataList.splitOn "." path
                    in
                        insertRulePath pathParts rule tree
...
updateConfigTreeFromRuleTree swTrace txName redeemerLog txSpecs ruleTree config =
    let
        basePath = txName ++ "." ++ getRedeemerScriptNameFromLog redeemerLog ++ "." ++ getRedeemerNameFromLog redeemerLog ++ "."
    in
    ...
    in
        ...
        debugTraceIf_
            ...
            ( "\nPath: "
                ...
                ++ "\nScore: "
                ++ show (msDirectFirst4 matchScore)
                ++ " directs in first 4, "
                ++ show (msPartialFirst4 matchScore)
                ++ " partial in first 4, "
                ++ show (msAnyFirst4 matchScore)
                ++ " any in first 4, "
                ++ show (msDirectRest matchScore)
                ++ " directs in rest, "
                ++ show (msPartialRest matchScore)
                ++ " partial in rest, "
                ++ show (msAnyRest matchScore)
                ++ " any in rest, "
                ++ "Total Length "
                ++ show (msLevelsMatched matchScore)
                ++ "\nRule: "
                ++ ( case matchingRule of
                        Just rule ->
                            "Matched. Outcome: "
                                ++ show (ruleExpectedOutcome rule)
                                ++ " (ErrorSource: "
                                ++ show (ruleErrorSource rule)
                                ++ ")"
...
findBestMatchRuleForConfigPath swTrace configPath ruleTree initialRulePath =
    let
        ...
        selectBestMatch =
            foldl
                ( \acc@(_, _, accScore) cur@(_, _, curScore) ->
        ...
        findValidMatches segment children =
            let
                ...
                isWildcardMatch pattern' segment' =
                    let
                        regexPattern = "^" ++ DataText.unpack (DataText.replace "*" ".*" (DataText.pack pattern')) ++ "$"
                    in
                        segment' Regex.=~ regexPattern
                ----------
            ...
            in
                ----------
                directMatch ++ anyMatch ++ partialMatches
        ----------
        ...
        findBestMatchHelper [] (RuleNode _ maybeRule) rulePath depth' =
            case maybeRule of
                Just rule ->
                    let
                        score = ruleScore rule -- Use the pre-calculated score directly from the rule
                    in
                        debugTraceIf_
                            swTrace
                            ( indent depth'
                                ++ "Matched rule at path: "
                                ++ rulePath
                                ++ "\n"
                                ++ indent depth'
                                ++ "Score: "
        ...
        findBestMatchHelper (x : xs) (RuleNode children' maybeRule) rulePath depth' =
            let
                matches = findValidMatches x children'
                ----------
                    ...
                    Just rule ->
                        ...
                        in
                            debugTraceIf_
                                swTrace
                                ( indent depth'
                                    ++ "Path continues but Matched rule at path: "
                                    ++ rulePath
                                    ++ "\n"
                                    ++ indent depth'
                                    ++ "Score: "
                ...
                results =
                    map
                        ( \(matchType, key, subTree) ->
                            debugTraceIf_
                                swTrace
                                ( indent depth'
                                    ++ "Checking rule "
                                    ++ (if P.null rulePath then key else rulePath ++ "." ++ key)
                                    ++ " (matches "
                                    ++ x
                                    ++ ", type: "
                                    ++ show matchType
            ...
            in
                selectBestMatch combinedResults
    in
