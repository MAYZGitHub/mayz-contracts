...
readMintingPolicy path file = do
    ...
    case policy' of
        Left _ -> P.error "Policy not found"
        Right policy -> do
...
{-# INLINEABLE writeCompiledCodeToBinaryFile #-}
-- Create file with compiled code
...
{-# INLINEABLE readCompiledCodeFromBinaryFile #-}
...
{-# INLINEABLE compiledCodeToProgramTerm #-}
...
{-# INLINEABLE applyPlutononyToMkValidator #-}
applyPlutononyToMkValidator ::  (BuiltinData -> BuiltinData -> BuiltinData -> ()) -> LedgerApiV2.Validator
applyPlutononyToMkValidator mkValidator  =
    Plutonomy.optimizeUPLC $
        Plutonomy.validatorToPlutus $
            Plutonomy.mkValidatorScript $$( PlutusTx.compile [|| mkValidator ||])
    -- LedgerApiV2.mkValidatorScript $$( PlutusTx.compile [|| mkValidator ||])

{-# INLINEABLE applyPlutononyToMkMintingPolicy #-}
applyPlutononyToMkMintingPolicy ::  (BuiltinData -> BuiltinData -> ()) -> LedgerApiV2.MintingPolicy
applyPlutononyToMkMintingPolicy mkPolicy  =
    Plutonomy.optimizeUPLC $
        Plutonomy.mintingPolicyToPlutus $
            Plutonomy.mkMintingPolicyScript $$( PlutusTx.compile [|| mkPolicy ||])

--------------------------------------------------------------------------------2

{-# INLINEABLE dataToBuiltinData #-}
dataToBuiltinData :: PlutusTx.Data -> BuiltinData
dataToBuiltinData  = TxBuiltins.dataToBuiltinData

{-# INLINEABLE stringHexToBuiltinData #-}
...
{-# INLINEABLE optimizeMintingPolicy #-}
