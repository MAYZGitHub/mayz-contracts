...
endPointFundPrepare T.PABFundPrepareParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    ---------------------
    let !investUnitValidator_Hash = T.pppInvestUnitValidator_Hash protocolPABParams
    ---------------------
    ...
    ---------------------
    let !tokenMAYZ_AC = T.pppTokenMAYZ_AC protocolPABParams
    ...
    ---------------------
        ...
        !holdingsIndex = 0
    ---------------------
        !fundDatum_Out = FundT.mkFund_Datum
            fundPolicy_CS
            fundFT_TN
            fundValidator_Hash
...
endPointFundUpdate T.PABFundUpdateParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    ---------------------
        !fundDatum_Out =
            FundT.FundDatum $ FundHelpers.mkUpdated_Fund_Datum_With_NormalChanges fundDatum_In
...
endPointFundHoldingAdd T.PABFundHoldingAddParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !redeemer_For_Consuming_FundDatum = FundT.ValidatorRedeemerFundHoldingAdd FundT.ValidatorRedeemerFundHoldingAddType
...
endPointFundDeposit T.PABFundDepositParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    ---------------------
    let !fundFT_TN = FundT.fdFundFT_TN fundDatum_In
    ---------------------
...
endPointFundWithdraw T.PABFundWithdrawParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !redeemer_For_Consuming_FundHoldingDatum = FundHoldingT.ValidatorRedeemerWithdraw $ FundHoldingT.ValidatorRedeemerWithdrawType now withdraw withdrawPlusCommissionsGetBack
...
endPointFundReIndexing T.PABFundReIndexingParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    let !fundValidator_Address = T.fppFundValidator_Address fundPABParams
        !fundValidator_AddressCardano = OffChainHelpers.addressToCardanoAddress T.networkId fundValidator_Address
    ---------------------
    let !investUnitValidator = T.pppInvestUnitValidator protocolPABParams
    ...
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !valueOf_TotalTokensToAdd = OnChainHelpers.flattenValueToValue [(cs, tn, am * total_Deposits_IU) | (cs, tn, am) <- tokensToAdd ]
...
endPointFundCollect_Protocol_Commission T.PABFundCollect_Protocol_CommissionParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !redeemer_For_Consuming_FundHoldingDatum = FundHoldingT.ValidatorRedeemerCollect_Protocol_Commission $ FundHoldingT.ValidatorRedeemerCollect_Protocol_CommissionType now withdraw
    PlutusContract.logInfo @P.String $ TextPrintf.printf "redeemer_For_Consuming_FundHoldingDatum: %s" (P.show redeemer_For_Consuming_FundHoldingDatum)
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !intervalOffset1 = 1000
...
endPointFundCollect_Delegators_Commission T.PABFundCollect_Delegators_CommissionParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !redeemer_For_Consuming_FundHoldingDatum = FundHoldingT.ValidatorRedeemerCollect_Delegators_Commission $ FundHoldingT.ValidatorRedeemerCollect_Delegators_CommissionType now withdraw
    PlutusContract.logInfo @P.String $ TextPrintf.printf "redeemer_For_Consuming_FundHoldingDatum: %s" (P.show redeemer_For_Consuming_FundHoldingDatum)
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !intervalOffset1 = 1000
...
endPointFundCollect_Managers_Commission T.PABFundCollect_Managers_CommissionParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !redeemer_For_Consuming_FundHoldingDatum = FundHoldingT.ValidatorRedeemerCollect_Managers_Commission $ FundHoldingT.ValidatorRedeemerCollect_Managers_CommissionType now withdraw
    PlutusContract.logInfo @P.String $ TextPrintf.printf "redeemer_For_Consuming_FundHoldingDatum: %s" (P.show redeemer_For_Consuming_FundHoldingDatum)
    PlutusContract.logInfo @P.String $ TextPrintf.printf "-------------------"
    ---------------------
    let !intervalOffset1 = 1000
