...
endPointProtocolPrepare T.PABProtocolPrepareParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    let
        ...
        -- TODO: problema con datum grande
        ---------------------
    ...
    let
        !protocolPABParams = ppppProtocolPABParams
    ...
    let
        !protocolPolicyID_Params = T.pppProtocolPolicyID_Params protocolPABParams
    ...
    let
        protocolPolicyID_TxOut = head [(t, ci) | (t, ci) <- DataMap.toList uTxOsAtUser, t == protocolPolicyID_TxOutRef]
    ...
    do
        let
            (lookupsTx_Mint_ProtocolID, tx_Mint_ProtocolID) = OffChainHelpers.mintToken_With_RefPolicyOrAttachedPolicy valueFor_Mint_ProtocolID (Just protocolPolicyID_TxOut) (Nothing :: Maybe LedgerApiV2.Redeemer) Nothing (Just protocolPolicyID)
            lookupsTx =
                LedgerConstraints.unspentOutputs uTxOsAtUser
...
endPointProtocolUpdate T.PABProtocolUpdateParams {..} = PlutusContract.handleError OffChainHelpers.handleContractError $ do
    ...
    let
        !protocolPABParams = ppupProtocolPABParams
    ...
    let
        !valueOf_ProtocolDatum_In = OffChainHelpers.getValueFromDecoratedTxOut $ (\(_, dec, _) -> dec) uTxO_With_ProtocolDatum
        !valueFor_ProtocolDatum_Out = valueOf_ProtocolDatum_In
        !protocolDatum_In = (\(_, _, dat) -> dat) uTxO_With_ProtocolDatum
    ...
    let
        !redeemer_For_Consuming_ProtocolDatum = T.ValidatorRedeemerDatumUpdate T.ValidatorRedeemerDatumUpdateType
    ...
    do
        let
            (lookupsTx_Consume_ProtocolDatum, tx_Consume_ProtocolDatum) =
                OffChainHelpers.mustSpendScriptOutput_With_RefPolicyOrAttachedPolicy' uTxO_With_ProtocolDatum redeemer_For_Consuming_ProtocolDatum scriptRef_With_ProtocolValidator' (Just protocolValidator)
        let
            lookupsTx =
                LedgerConstraints.unspentOutputs uTxOsAtUser
