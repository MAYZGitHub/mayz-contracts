...
-- Any change in the logic, datum or redeemer must change the version of the buyOrderVersion on the Protocol.BuyOrder.Types

--------------------------------------------------------------------------------2

{-# INLINEABLE mkPolicyID #-}
mkPolicyID :: T.PolicyParams -> BuiltinData -> BuiltinData -> ()
mkPolicyID (T.PolicyParams !protocolPolicyID_CS !buyOrder_Validator_Hash) !redRaw !ctxRaw =
    let
        ------------------
                    ...
                    where
                        ...
                        !inputs_Own_TxOuts =
                            [ LedgerApiV2.txInInfoResolved txInfoInput | !txInfoInput <- LedgerApiV2.txInfoInputs info, let
                                                                                                                        ...
                                                                                                                        in
                                                                                                                            OnChainHelpers.isScriptAddress address && address == buyOrder_Validator_Address
                            ]
                        ------------------
                        !outputs_Own_TxOuts =
                        ...
                        !isCorrect_Output_BuyOrder_Datum =
                            ...
                            in
                                buyOrder_Datum_Out
                                    `OnChainHelpers.isUnsafeEqDatums` buyOrder_Datum_Out_Control
                        ...
                        !isCorrect_Output_BuyOrder_Value =
                            ...
                            in
                                ---------------------
                                valueOf_BuyOrder_Out
                                    `OnChainHelpers.isEqValue` valueFor_BuyOrder_Datum_Out_Control
        ...
        validateAllRedeemersButDelete
            ...
            where
                ...
                isCorrect_Output_BuyOrder_Value_NotChanged =
                    let
                        !valueFor_BuyOrder_Out_Control = LedgerApiV2.txOutValue input_TxOut_BeingValidated
                    in
                        valueOf_BuyOrder_Out `OnChainHelpers.isEqValue` valueFor_BuyOrder_Out_Control
                ----------------
                ...
                convertToMap (T.InvestUnit tokens) = foldl insertToken TxAssocMap.empty tokens
                    where
                        insertToken acc (cs, tn, i) =
                            let
                                innerMap = DataMaybe.fromMaybe TxAssocMap.empty (TxAssocMap.lookup cs acc)
                                updatedInnerMap = TxAssocMap.insert tn (maybe i (+ i) (TxAssocMap.lookup tn innerMap)) innerMap
                            in
                                TxAssocMap.insert cs updatedInnerMap acc
                ------------------
                ...
                validateRedeemerUpdateStatus (T.ValidatorRedeemerUpdateStatus (T.ValidatorRedeemerUpdateStatusType !newStatus)) =
                    ...
                    where
                        ...
                        !isCorrect_Output_BuyOrder_Datum_With_StatusChanged =
                            ...
                            in
                                buyOrder_Datum_Out `OnChainHelpers.isUnsafeEqDatums` buyOrder_Datum_Out_Control
                ------------------
                ...
                validateRedeemerUpdateMinADA (T.ValidatorRedeemerUpdateMinADA _) =
                    ...
                    where
                        ...
                        isCorrect_Output_BuyOrder_Value_With_MinADAChanged =
                            let
                                !valueFor_BuyOrder_Out_Control = LedgerApiV2.txOutValue input_TxOut_BeingValidated <> LedgerAda.lovelaceValueOf (newMinADA - T.bodMinADA buyOrder_Datum_In)
                            in
                                valueOf_BuyOrder_Out `OnChainHelpers.isEqValue` valueFor_BuyOrder_Out_Control
                ...
                validateRedeemerFillOrder (T.ValidatorRedeemerFillOrder (T.ValidatorRedeemerFillOrderType !rfoAmount_Tokens !rfoAmount_FT !rfoCommission_FT !rfoOracle_Data !rfoOracle_Signature)) =
                    ...
                    traceIfFalse "not isOrderOpen" isOrderOpen
                        ...
                        && traceIfFalse "not isCorrect_Oracle_InRangeTime" (OnChainHelpers.isCorrect_Oracle_InRangeTime info (T.odTime rfoOracle_Data) (ProtocolT.pdOracleData_Valid_Time protocolDatum_In))
                            ...
                            PlutusTx.Prelude.foldl
                                ( \acc cs ->
                                    PlutusTx.Prelude.foldl
                                        ( \innerAcc (tn, amount) ->
                                            let
                                                price = DataMaybe.fromMaybe 0 (TxAssocMap.lookup cs oraclePrices >>= TxAssocMap.lookup tn)
                                            in
                                                innerAcc + (price * amount)
                                        )
                                        0
                                        (TxAssocMap.toList $ DataMaybe.fromMaybe TxAssocMap.empty (TxAssocMap.lookup cs swapTokens))
                                        + acc
                                )
                                0
                                (TxAssocMap.keys swapTokens)
                        ------------------
...
mkUpdated_BuyOrder_Datum_With_FillOrder !buyOrder_Datum_In !amount_FT _ !commission_FT =
    buyOrder_Datum_In
        { T.bodFT_Received = T.bodFT_Received buyOrder_Datum_In + (amount_FT - commission_FT)
...
mkWrappedPolicyID protocolPolicyID_CS buyOrder_Validator_Hash = mkPolicyID params
    where
        params =
            T.PolicyParams
